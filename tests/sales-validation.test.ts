...
describe("sales timing server validation", () => {
  ...
  it("accepts a configurable timing rule", () => { expect(timingRuleInputSchema.parse({ organizationId: ids.organizationId, name: "rule", configuration: { eventType: "funding", offsetDays: 30 } }).configuration.cooldownDays).toBe(0); });
  it("rejects excessive or negative rule values", () => { expect(() => timingRuleInputSchema.parse({ organizationId: ids.organizationId, name: "rule", configuration: { eventType: "funding", offsetDays: -1 } })).toThrow(); });
});
